...
function getYouTubeVideoId(value) {
  ...
  try {
    ...
    if (videoParameter && /^[a-zA-Z0-9_-]{11}$/.test(videoParameter))
...
function updateSongInformation() {
  ...
  if (!song) return;
  if (songTitle)
    songTitle.textContent = song.title || song.name || "Unknown Song";
  if (songArtist)
    songArtist.textContent = song.artist || song.singer || "Unknown Artist";
  if (currentSongNumber)
    currentSongNumber.textContent = String(currentSongIndex + 1).padStart(
      2,
      "0",
    );
...
function renderSongs(categoryFilter = "all", title = "All Songs") {
  ...
  const displaySongs = list
    ...
    .filter((song) => {
      ...
      return (
        song.category &&
        song.category.toLowerCase() === categoryFilter.toLowerCase()
      );
    });
...
function renderPlaylist() {
  const playlistCategories = document.querySelectorAll(
...
function createYouTubePlayer() {
  ...
  const firstVideoId = getYouTubeVideoId(
    firstSong.youtubeId || firstSong.videoId || firstSong.url,
  );
  if (!firstVideoId) {
...
function loadExperienceSettings() {
  try {
    ...
    if (saved && typeof saved === "object")
      experienceSettings = { ...defaultExperience, ...saved };
  } catch (error) {
    console.warn("Experience settings load nahi hui.", error);
  }
}

function saveExperienceSettings() {
  try {
    localStorage.setItem(EXPERIENCE_KEY, JSON.stringify(experienceSettings));
  } catch (error) {
    console.warn("Experience settings save nahi hui.", error);
  }
}
...
function setNightMode(enabled) {
  ...
  if (themeToggle) {
    themeToggle.innerHTML = experienceSettings.night
      ? "🌙 <span>Night</span>"
      : "☀️ <span>Day</span>";
    themeToggle.title = experienceSettings.night
      ? "Switch to Day Mode"
...
async function setDriveMode(enabled) {
  ...
  try {
    ...
    console.warn("Fullscreen activate nahi hua.", error);
  }
}
...
========================================================= */
if (rainToggle)
  rainToggle.addEventListener("click", () => setRain(!experienceSettings.rain));
if (lightsToggle)
...
if (driveToggle)
  driveToggle.addEventListener("click", () =>
    setDriveMode(!experienceSettings.drive),
  );
